 ...
 */
export const createTelemetryMiddleware = (): Middleware => async (data: MessageData, context: MessageContext, next: () => Promise<void>) => {
...
function extractEventData(_type: EventMessage, value: any): Record<string, any> {
  ...
  if (typeof value === 'object') {
    ...
    if (value.sessionId) {data.has_session_id = true;}
    if (value.modelName) {data.has_model_name = true;}
    if (value.title) {data.has_title = true;}
    if (value.filePath) {data.has_file_path = true;}
    if (value.startLine !== undefined) {data.has_line_selection = true;}
