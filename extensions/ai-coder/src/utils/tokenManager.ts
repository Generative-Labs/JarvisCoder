...
class TokenManager {
   ...
   * @returns {TokenManager} The singleton instance
   */
   ...
   * @param {vscode.SecretStorage} storage - The VS Code secret storage
   */
   ...
   * @returns {Promise<string | null>} The current refresh token or null if not available
   */
   ...
   * @param {number} [thresholdSeconds=300] - The threshold in seconds to consider as "soon"
   * @returns {boolean} True if the token is expiring soon or not set
   */
   ...
   * @returns {boolean} True if the token is expired or not set
   */
   ...
   * @param {(tokenInfo: { token: string; refreshToken: string; expiresAt: number }) => void} [onNewTokenInfo] - Callback when a new token is obtained
   * @param {boolean} [hardRefresh] - Whether to force a hard refresh of the token
   * @returns {Promise<string | null>} The current or new token
   * @throws {Error} If the refresh token is expired or invalid
   */
  public async refreshTokenIfNeeded(
    onNewTokenInfo?: (tokenInfo: {
      token: string;
      refreshToken: string;
      expiresAt: number;
    }) => void,
