 ...
 * @param text - The document content to parse, expected to contain metadata between '---' delimiters
 * @returns An object containing the parsed document parts
...
export function parseDocumentContent(text: string): {
	...
	if (firstDelimiterIndex === -1) {
		throw new Error('Invalid prompt format: Missing metadata section (should start with ---)');
	}

	const secondDelimiterIndex = text.indexOf('---', firstDelimiterIndex + 3);
	if (secondDelimiterIndex === -1) {
		throw new Error('Invalid prompt format: Missing metadata closing delimiter (---)');
	}

	// Extract the metadata section between the two delimiters
	const metadataSection = text.substring(firstDelimiterIndex + 3, secondDelimiterIndex).trim();
