...
export class CodeDiffController {
	 ...
	 * @param leftFileUri - The URI of the original file
	 * @param rightFileUriOrContent - Either the URI of the modified file or the content to compare
	 */
	constructor(
		...
		if (typeof rightFileUriOrContent === 'string') {
			...
			this.currentRightFileUri = this.registerInMemoryDocument(rightFileUriOrContent);
		} else {
	 ...
	 * @param content - Text content for the in-memory document
	 * @returns URI of the in-memory document
	 */
	private registerInMemoryDocument(content: string): vscode.Uri {
		...
		this.contentProvider = vscode.workspace.registerTextDocumentContentProvider(scheme, {
			/**
	 ...
	 * @param onAccept - Optional callback function to execute after changes are accepted
	 * @returns A promise that resolves when the changes have been accepted
	 */
	async acceptChanges(onAccept?: () => void): Promise<void> {
		...
		if (this.inMemoryContent !== undefined) {
			...
			const rightDoc = await vscode.workspace.openTextDocument(this.currentRightFileUri);
			rightContent = rightDoc.getText();
		}

		// Write the content to the left file
		const leftDoc = await vscode.workspace.openTextDocument(this.currentLeftFileUri);
	 ...
	 * @param onReject - Optional callback function to execute after changes are rejected
	 * @returns A promise that resolves when the changes have been rejected
	 */
