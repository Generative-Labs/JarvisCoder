...
export class FileTracker implements vscode.Disposable {
      ...
      if (!this.shouldSkipFile(filePath)) {
        ...
        try {
          ...
          if (result.changed && result.metadata) {
            logger.debug(`File changed after save: ${filePath}, metadata stored: ${this.changedFiles.has(filePath)}`);
    ...
    try {
      for (const folder of workspaceFolders) {
        for (const patternStr of SOURCE_FILE_PATTERNS) {
          ...
          const excludePattern = new vscode.RelativePattern(folder, `{${EXCLUDED_DIRECTORIES.join(',')}}`);
  ...
  private async checkFile(uri: vscode.Uri): Promise<FileChangeResult> {
    ...
    try {
      ...
      const workspaceLastSync = workspaceFolder ?
        await FileMetadataStore.getWorkspaceLastSyncTime(workspaceFolder) : 0;

      // Check if file has actually changed
      const hasChanged = !existingMetadata ||
  ...
  private shouldIgnoreFiles(filePath: string): boolean {
    ...
    return ignorePatterns.some(pattern =>
      fileName === pattern || filePath.endsWith(pattern)
    );
  }
  ...
  private shouldSkipFile(filePath: string): boolean {
    ...
    return !SOURCE_FILE_PATTERNS.some(pattern =>
   ...
   * @returns {string} The workspace folder name
   */
   ...
   * @returns {Promise<FileMetadata[]>} Array of changed file metadata that need sync
   */
  public async checkForChanges(): Promise<FileMetadata[]> {
    try {
      // Get all files that need sync
      const filesToSync = Array.from(this.changedFiles.values())
        .filter(metadata => {
          ...
          return !existingMetadata?.lastSynced ||
            existingMetadata.lastSynced < metadata.lastModified ||
            existingMetadata.md5Hash !== metadata.md5Hash;
        });

      if (filesToSync.length > 0) {
        logger.debug(`Found ${filesToSync.length} files that need sync: ${filesToSync.map(f => f.path).join(', ')}`);
