...
export class ImportResolver {
	 ...
	 * @returns List of absolute paths to imported files
	 */
	public static async resolveImports(
		...
		try {
			...
			logger.error(
				`Error resolving imports for ${filePath}:`,
	...
	private static resolveJsImports(filePath: string, fileContent: string): string[] {
		...
		while ((match = es6ImportRegex.exec(fileContent)) !== null) {
			...
			if (this.isLocalImport(importPath)) {
				const resolvedPath = this.resolveLocalImportPath(baseDir, importPath, [
					'.ts',
					'.tsx',
					'.js',
					'.jsx',
	 ...
	 * @param extensions - Possible file extensions to try
	 ...
	 * @param baseDir - Base directory to resolve from
	 * @param moduleName - Python module name to resolve
	 * @returns Absolute path to the module file or null if not found
	 */
	 ...
	 * @param moduleName - Python module name to check
	 * @returns True if the module is a built-in module
	 */
	private static isPythonBuiltinModule(moduleName: string): boolean {
		...
		return builtinModules.includes(moduleName) || moduleName.split('.')[0].startsWith('_');
	}
}
