...
`;
    ...
    suite('matchesPattern', () => {
        test('should match simple filename patterns', () => {
            ...
            testCases.forEach(({ path, pattern, expected }) => {
                ...
                assert.strictEqual(matches, expected, `Pattern ${pattern} should ${expected ? 'match' : 'not match'} ${path}`);
            });
        });
    ...
    suite('real-world patterns', () => {
        test('should handle typical Node.js gitignore patterns', () => {
            ...
            testCases.forEach(({ path, pattern, expected }) => {
                const matches = testHandler.matchesPattern(path, pattern);
                assert.strictEqual(matches, expected, `Node.js pattern ${pattern} should ${expected ? 'match' : 'not match'} ${path}`);
            });
        });
        ...
        test('should handle typical Python gitignore patterns', () => {
            ...
            testCases.forEach(({ path, pattern, expected }) => {
                const matches = testHandler.matchesPattern(path, pattern);
                assert.strictEqual(matches, expected, `Python pattern ${pattern} should ${expected ? 'match' : 'not match'} ${path}`);
            });
        });
        ...
        test('should handle IDE and OS patterns', () => {
            ...
            testCases.forEach(({ path, pattern, expected }) => {
                const matches = testHandler.matchesPattern(path, pattern);
                assert.strictEqual(matches, expected, `IDE/OS pattern ${pattern} should ${expected ? 'match' : 'not match'} ${path}`);
            });
        });
    });
